...
const generateDummyRequests = () => {
  ...
  return Array.from({ length: 25 }, (_, index) => {
    ...
    submittedDate.setDate(
      submittedDate.getDate() - Math.floor(Math.random() * 60),
    );
    ...
    return new VendorRequest({
        ...
        status !== "pending"
          ? new Date(
              submittedDate.getTime() +
                86400000 * Math.floor(Math.random() * 5),
            ).toISOString()
      ...
      vendor: {
        ...
        role: "vendor",
        email_verified_at:
          Math.random() > 0.3 ? submittedDate.toISOString() : null,
        created_at: submittedDate.toISOString(),
      },
    });
  });
...
export const vendorRequestService = {
  ...
  approve: async (requestId) => {
    try {
      const response = await fetch(
        `${BASE_URL}/admin/services/${requestId}/approve`,
        {
          method: "PUT",
          headers: getHeaders(),
        },
  ...
  reject: async (requestId, reason) => {
    try {
      const response = await fetch(
        `${BASE_URL}/admin/services/${requestId}/reject`,
        {
          method: "PUT",
          headers: getHeaders(),
