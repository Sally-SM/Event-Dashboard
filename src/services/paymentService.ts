...
const generateDummyData = () => {
  const payments = Array.from({ length: 28 }, (_, index) => {
    ...
    return new Payment({
      ...
      amount: `${amount}.00`,
      refund_amount:
        status === "refunded" ? `${Math.floor(amount * 0.5)}.00` : null,
      status,
      paid_at: paidAt,
  ...
  const totalRefunded = refunded.reduce(
    (s, p) => s + p.refundAmountAsNumber,
    0,
  );
  const totalInvoices = payments.reduce(
    (s, p) => s + p.invoiceTotalAsNumber,
    0,
  );
...
export const paymentService = {
  ...
  getFinanceData: async (filters = {}) => {
    ...
    try {
      ...
      console.warn(
        "API fetch failed, using dummy finance data:",
        error.message,
      );
    ...
    try {
      ...
      if (json?.status === "success" && json?.data) {
        return {
          ...
          payment: Payment.fromApi(json.data),
        };
      }
